...
```bash
inblog posts tags <postId> --json
inblog posts add-tags <postId> --tag-ids 1,2,3 --json
inblog posts remove-tag <postId> <tagId> --json
```

### 포스트-저자 관계

```bash
inblog posts authors <postId> --json
inblog posts add-authors <postId> --author-ids uuid1,uuid2 --json
inblog posts remove-author <postId> <authorId> --json
```
...
| 필드 | 설명 |
|------|------|
| `name` | 태그 이름 (API에서 name으로 노출) |
| `slug` | URL 슬러그 |
| `priority` | 정렬 우선순위 (낮을수록 먼저) |
...
inblog search-console disconnect
